...
class ConversationConverter:
    ...
    def convert_to_training_format(self, grouped_utterances: List[Dict[str, str]]) -> Dict:
        """
    ...
    def convert_file(self, input_path: str, output_path: Optional[str] = None) -> str:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            # Write each conversation as a separate JSONL entry
...
def main():
    ...
    INPUT_FILE = "Ep.10_final.txt"  # Change this to your file path

    # Output file path (optional - will auto-generate if None)
    OUTPUT_FILE = None  # e.g., "output.json" or None for auto-generation

    # Speaker configurations
    THERAPIST_SPEAKERS = ["Speaker 5"]  # List of speaker IDs for therapist

    PARTNER_A_SPEAKERS = ["Speaker 3", "Speaker 7", "Speaker 15"]  # List of speaker IDs for Partner A

    PARTNER_B_SPEAKERS = ["Speaker 4", "Speaker 6", "Speaker 8", "Speaker 9"]  # List of speaker IDs for Partner B

    EXCLUDED_SPEAKERS = ["Speaker 1", "Speaker 2"]  # List of speaker IDs to exclude from output
    ...
    try:
        ...
        with open(output_path, 'r', encoding='utf-8') as f:
            ...
            print(f"  - {len([c for c in conversations if c['from'] == 'human'])} human messages")
            print(f"  - {len([c for c in conversations if c['from'] == 'gpt'])} assistant messages")
